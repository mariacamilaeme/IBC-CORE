  ...
  return (
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: T.ink }}>Reporte de Pagos</h1>
          <p style={{ fontSize: 13, color: T.inkMuted, marginTop: 4 }}>
            {CATEGORIES.find((c) => c.key === activeCategory)?.label} — <span style={{ fontWeight: 600, color: T.inkSoft }}>{filtered.length} registros</span>
          </p>
        </div>
      ...
      <div style={{ borderRadius: T.radiusMd, border: "1px solid " + T.borderLight, overflow: "hidden", boxShadow: T.shadowMd }}>
        {loading ? (
          <div className="flex items-center justify-center py-20">
            ...
            <span style={{ marginLeft: 8, fontSize: 13, color: T.inkMuted }}>Cargando pagos...</span>
          </div>
        ) : filtered.length === 0 ? (
