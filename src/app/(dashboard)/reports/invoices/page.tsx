...
export default function InvoicesReportPage() {
  ...
  const handleChinaExport = async () => {
    try {
      ...
      // ROW 2
      const r2 = ws.addRow([""]); r2.height = 5;
  ...
  return (
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: T.ink }}>Reporte de Facturas</h1>
          <p style={{ fontSize: 13, color: T.inkMuted, marginTop: 4 }}>
            {activeTab === "china" ? "Facturas China" : "Facturación Comercial"} — <span style={{ fontWeight: 600, color: T.inkSoft }}>{currentData.length} registros</span>
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button size="sm" className="h-9 gap-1.5 rounded-xl" onClick={activeTab === "china" ? handleChinaExport : handleCommExport} style={{ background: T.gradientPrimary, border: "none", boxShadow: T.shadowMd, color: "white" }}>
      ...
      <div style={{ borderRadius: T.radiusMd, border: "1px solid " + T.borderLight, overflow: "hidden", boxShadow: T.shadowMd }}>
        {loading ? (
          <div className="flex items-center justify-center py-20">
            ...
            <span style={{ marginLeft: 8, fontSize: 13, color: T.inkMuted }}>Cargando facturas...</span>
          </div>
      ...
      <p style={{ fontSize: 11, color: T.inkLight, textAlign: "center" }}>Vista previa · El archivo Excel contiene todas las columnas completas</p>
    </div>
    </div>
  );
}
